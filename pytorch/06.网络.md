...
- [06.网络](#06网络)
  - [1. 网络op](#1-网络op)
    - [a. 全连接（nn.Linear()）](#a-全连接nnlinear)
    - [b. 卷积（nn.Conv2，nn.Conv3d）](#b-卷积nnconv2nnconv3d)
    - [c. 时序op（nn.RNN(), nn.LSTM(), nn.GRU()）](#c-时序opnnrnn-nnlstm-nngru)
    - [c. 激活函数（nn.ReLU(), nn.Sigmoid(), nn.Tanh(), nn.LeakyReLU()）](#c-激活函数nnrelu-nnsigmoid-nntanh-nnleakyrelu)
    - [x. op组合（nn.Sequential()）](#x-op组合nnsequential)
  - [2. 定义网络](#2-定义网络)
    - [a. 定义 ResNet](#a-定义-resnet)
    - [b. 定义 capsNet](#b-定义-capsnet)
    - [c. 定义 ResNet3D\_VAE](#c-定义-resnet3d_vae)
    - [c. 定义 LSTM](#c-定义-lstm)
      - [不定长输入](#不定长输入)
    - [c. 定义 NeXtVLAD](#c-定义-nextvlad)
    - [d. 定义 Transformer](#d-定义-transformer)
    - [e. 定义 Diffusion](#e-定义-diffusion)
...
https://github.com/w-garcia/NeXtVLAD.pytorch/tree/master
...
https://github.com/hyunwoongko/transformer/tree/master/models
